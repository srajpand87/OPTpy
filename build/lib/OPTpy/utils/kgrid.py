...
class KgridTask(Task):
    ...
    def __init__(self,
        ...
        self.dirname = path.dirname(rootname)
    ...
    @property
    def new_dir(self):
        return self.dirname and not path.exists(self.dirname)
    ...
    def run(self):
        try:
            subprocess_call([self.executable, self.inputname,
...
def get_kpt_grid(structure, ngkpt,
    ...
    dirname = path.dirname(rootname)
    new_dir = dirname and not path.exists(dirname)
    ...
    if new_dir:
        ...
        subprocess_call(['mkdir', '-p', dirname])
    ...
    # Run kgrid.x
    try:
        subprocess_call([executable, inputname, outputname, logname])
    except OSError as E:
        raise OSError(str(E) + '\n' +
